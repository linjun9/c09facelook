...
router.post("/room/:id/enter/", isAuthenticated, function(req, res, next) {
    ...
    Rooms.findOne({ _id: id }, function(err, room) {
        ...
        Rooms.updateOne({ _id: id }, { users: room.users, owner: room.users[0] }, function(err, result) {
            if (err) return res.status(500).end(err);
...
router.post("/room/:id/leave/", isAuthenticated, function(req, res, next) {
    ...
    Rooms.findOne({ _id: id }, function(err, room) {
        ...
        if (room.users.length === 0) {
            ...
            Rooms.updateOne({ _id: id }, { users: room.users, owner: room.users[0] }, function(
                err,
                result
            ) {
                if (err) return res.status(500).end(err);
...
router.ws("/rooms/:roomId", function(ws, req) {
    ...
    if (!roomConnection) {
        ...
        let newPlayer = { playerId: req.session.user._id, point: 0, isReady: false };
        roomState.players.push(newPlayer);
        roomConnection = { roomId: req.params.roomId, connections: [ws], roomState: roomState };
    ...
    ws.on('message', function(message) {
        ...
        switch (type) {
            case 'leave':
                roomConnection.roomState.players = roomConnection.roomState.players.filter(p => p.playerId !== req.session.user._id);
                roomConnection.connections = roomConnection.connections.filter(c => c !== ws);
            ...
            case 'ready':
                ...
                let readyPlayers = roomConnection.roomState.players.filter(p => p.isReady);
                let readyPlayerIds = readyPlayers.map(p => p.playerId);
                if (!(readyPlayerIds.includes(req.session.user._id))) {
                    let p = roomConnection.roomState.players.find(p => p.playerId === req.session.user._id);
                    p.isReady = true;
                    p.point = 0;
                    // roomConnection.roomState.players.push(newPlayer);
                }
                let numOfReady = roomConnection.roomState.players.filter(p => p.isReady).length;
                if (numOfReady === 1) {
                    roomConnection.roomState.gameState = "wait";
                    roomConnection.connections.forEach((player) => {
                        player.send(JSON.stringify({
                    ...
                    roomConnection.timeUpdater = setInterval(() => {
                        roomConnection.roomState.timeleft = roomConnection.roomState.timeleft - 1;
                    ...
                    roomConnection.timeOut = setTimeout(() => {
                        ...
                        let maxPoint = Math.max.apply(Math, roomConnection.roomState.players.map(function(o) { return o.point; }));
            ...
            case 'update':
                ...
                let targetPlayer = roomConnection.roomState.players.find(p =>
